...
class Solution {
    ...
    private int[] solve(int[][] board, int r1, int c1, int r2, int c2) {
        // 현재 위치에서 게임이 끝났는지 확인
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int[] result = solve(board, r2, c2, nr, nc); // 재귀적으로 다음 턴 계산
    ...
    private boolean isFinished(int[][] board, int r, int c) {
        for (int i = 0; i < 4; i++) {
            int nr = r + dr[i];
            int nc = c + dc[i];
    ...
    private boolean inRange(int[][] board, int r, int c) {
