...
class Solution {
    public int solution(final String original) {
        ...
        for (int count = 1; count < original.length(); count++) {
            final Deque<CodingTest.Programmers.Solution.Word> deque = new ArrayDeque<>();
            ...
            for (; searchIdx <= original.length() - count; searchIdx += count) {
                ...
                //deque가 비어있으면 추가
                if (deque.isEmpty()) {
                    deque.addLast(new CodingTest.Programmers.Solution.Word(str, 1));
                    continue;
                }

                final CodingTest.Programmers.Solution.Word peek = deque.peekLast();
            ...
            for (final CodingTest.Programmers.Solution.Word word : deque) {
                if (word.getCount() == 1) {
